...
// applyDetailedMetricsBytesSentAndTransmission updates the detailed metrics for bytes sent and transmission time.
func (cmd *baseCommand) applyDetailedMetricsBytesSentAndTransmission(ifc command, bytesSent int, startTime time.Time) {
	if cmd.node != nil && cmd.node.cluster != nil && cmd.node.cluster.MetricsEnabled() {
		end := uint64(time.Since(startTime).Microseconds())
		for namespace := range *ifc.getNamespace() {
			if namespace != "" {

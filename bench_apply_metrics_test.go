...
// fakeCommand implements the minimal parts of command used by applyDetailedMetricsBytesSentAndTransmission.
...
func BenchmarkApplyDetailedMetricsBytesSentAndTransmission(b *testing.B) {
	cmd, fcmd := newStub()

	// bytesSent value to simulate.
	bytesSent := 100
	//cmd.applyDetailedMetricsBytesSentAndTransmission(fcmd, bytesSent, time.Now())
